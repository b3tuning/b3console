 ...
 *              b3tuning@gmail.com
 *
 *  SerialPortTest is part of b3console
 *
...
public class SerialPortTest {
	...
	static public void main(String[] args) {
		...
		System.out.println("\nPre-setting RTS: " + (ubxPort.setRTS()
		                                            ? "Success"
		                                            : "Failure"));
		...
		readSemiBlocking(ubxPort, readBuffer);
		...
		System.out.println("Reopening " + ubxPort.getSystemPortName() + ": " + ubxPort.getDescriptivePortName() + ": " +
		                   openedSuccessfully);
		if (!openedSuccessfully) {
			return;
		}
	...
	private static void addDataListener(SerialPort ubxPort) {
	...
	private static void readSemiBlocking(SerialPort ubxPort, byte[] readBuffer) {
		try {
			for (int i = 0; i < 3; ++i) {
				System.out.println("\nReading #" + i);
