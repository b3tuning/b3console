...
import com.b3tuning.b3console.service.module.door.config.DoorConfigFlat;
...
//			log.entry();
			FXFormGenerator
					.forModel(new DoorConfigFlat())
					.showAsDialog(o -> {

						// Callback is executed after the form has been filled
						log.error("Now DoorConfig object is updated with form data");
					});
			Node fxForm = new FXForm(new DoorConfigFlat());
			form.getChildren().add(fxForm);
