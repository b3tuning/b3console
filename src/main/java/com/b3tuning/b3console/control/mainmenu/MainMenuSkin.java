...
import com.b3tuning.b3console.control.mainmenu.MainMenuItemModel.Mode;
import com.b3tuning.b3console.control.mainmenu.MainMenuItemModel.Position;
...
public class MainMenuSkin extends SkinBase<MainMenu> implements Skin<MainMenu> {
	...
	@Getter
	private ToggleGroup toggleGroup;        // ensures only 1 ToggleButton can be selected at any time
	...
	private void populateMenuItems() {
		...
		for (MainMenuItemModel menuItem : getSkinnable().getMenuItems()) {
			...
			if (Position.TOP.equals(menuItem.getPosition())) {
	...
	private AnchorPane buildMenuItem(MainMenuItemModel model) {
		...
		if (Mode.APP.equals(model.getMode())) {
			...
			l.getStyleClass().add(MENU_ITEM_STYLE_CLASS);
